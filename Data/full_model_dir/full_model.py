...
# Feature Selection
target = features['ppg']
X_data = features.drop(['ppg'], axis = 1)

# Train-Test Split
X_train, X_test, y_train, y_test = train_test_split(X_data.drop(['nbapersonid'], axis=1), target, test_size=0.2)
...
full_scaled = scaler.transform(X_data.drop(['nbapersonid'], axis=1))

full_preds = voting_reg.predict(full_scaled)

#Calcualte and predict
pred_df = pd.DataFrame(data = {
    'nbapersonid': X_data['nbapersonid'],
